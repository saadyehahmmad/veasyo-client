...
async function showMainMenu(): Promise<void> {
  ...
  console.log('\n');
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║     🎯 PC Agent - Interactive Management                 ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
  console.log('');
...
async function configureSettings(): Promise<void> {
  ...
  if (fs.existsSync(envPath)) {
    ...
    currentBackendUrl = process.env.BACKEND_URL || 'http://localhost:3000';
  ...
  console.log(`   Tenant ID: ${currentTenantId || '(not set)'}`);
  console.log(`   Printer IP: ${currentPrinterIp || '(not set)'}`);
  ...
  // Backend URL
  let backendUrl = await question(`Backend URL (current: ${currentBackendUrl}, press Enter to keep): `);
  if (!backendUrl.trim()) backendUrl = currentBackendUrl;
  while (!backendUrl.trim().match(/^https?:\/\/.+/)) {
    printError('Invalid URL format. Use format: http://hostname:port or https://hostname:port');
    backendUrl = await question(`Backend URL (current: ${currentBackendUrl}): `);
    if (!backendUrl.trim()) backendUrl = currentBackendUrl;
  }

  // Tenant ID
  let tenantId = await question(`Tenant ID (current: ${currentTenantId || '(not set)'}, press Enter to keep): `);
  if (!tenantId.trim()) tenantId = currentTenantId;
  while (!tenantId.trim()) {
...
async function uninstallService(): Promise<void> {
  ...
  const confirm = await question('⚠️  Are you sure you want to uninstall the service? (y/N): ');
...
async function checkStatus(): Promise<void> {
  ...
  try {
    ...
    const envPath = path.join(process.cwd(), '.env');
    if (!fs.existsSync(envPath)) {
      printError('No .env file found. Please run setup wizard first.');
      return;
    }
    ...
    console.log('   - Check logs for connection status and print job history');
    console.log('   - Log file: logs/pc-agent.log');
    ...
    if (error.code === 'ENOENT') {
      ...
      console.log('');
      printInfo('To configure:');
      console.log('   - Option 1: Setup Wizard');
      console.log('   - Option 2: Configure Settings');
    } else {
