...
export async function runSetupWizard(): Promise<void> {
  console.log('\n');
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║     🎯 PC Agent Setup Wizard                             ║');
  console.log('║     Complete setup in just a few steps                   ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
  console.log('');
  ...
  console.log('');
  console.log('🎉 PC Agent is ready to use!');
  console.log('');
  console.log('📝 Next Steps:');
  console.log('   1. Check status: npm run cli (option 6)');
  console.log('   2. View logs: logs/pc-agent.log');
  console.log('   3. Configure in Waiter Admin Panel:');
  console.log('      - Go to Admin Panel > Integrations > Printer');
  console.log('      - Enter PC Agent IP (this PC\'s IP address)');
  console.log('      - Enter PC Agent Port: ' + config.PORT);
  console.log('      - Test connection and save');
  console.log('');
  console.log('💡 Tips:');
  ...
  try {
    ...
    for (const name of Object.keys(interfaces)) {
      for (const iface of interfaces[name] || []) {
        if (iface.family === 'IPv4' && !iface.internal && !foundIp) {
          console.log('💡 Your PC\'s IP Address (for Admin Panel):');
...
async function collectConfiguration(): Promise<Record<string, string>> {
  ...
  // Server Port
  let port = await question('📡 Server Port (default: 3001): ');
  if (!port.trim()) port = '3001';
  while (!validatePort(port)) {
    printError('Invalid port. Must be between 1 and 65535.');
    port = await question('Server Port (default: 3001): ');
    if (!port.trim()) port = '3001';
  }
  config.PORT = port;
  printSuccess(`Server port set to: ${port}`);
  ...
  // Advanced (optional)
  console.log('\n⚙️  Advanced Settings (Optional - press Enter for defaults):');
